...
class StoreForwardManager {
    ...
    fun cacheMessage(packet: BitchatPacket, messageID: String) {
        ...
        val recipientPeerID = packet.recipientID?.let { recipientID ->
            recipientID.joinToString("") { "%02x".format(it) }
        }
        ...
        if (isForFavorite) {
            // Store in favorite queue (atomic computeIfAbsent + synchronized list)
            val queue = favoriteMessageQueue.computeIfAbsent(recipientPeerID) {
        ...
        managerScope.launch {
            ...
            val recipientMessages = messageCache.filter { storedMessage ->
                ...
                storedMessage.packet.recipientID?.let { recipientID ->
                    recipientID.joinToString("") { "%02x".format(it) } == peerID
                } == true
            }
