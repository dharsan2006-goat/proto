...
class PowerManager(private val context: Context) : LifecycleEventObserver {
    ...
    private var powerJob = SupervisorJob()
    private var powerScope = CoroutineScope(Dispatchers.IO + powerJob)
    ...
    fun start() {
        ...
        // Re-create scope if previously cancelled
        if (powerJob.isCancelled) {
            powerJob = SupervisorJob()
            powerScope = CoroutineScope(Dispatchers.IO + powerJob)
        }
        startDutyCycle()
    }
