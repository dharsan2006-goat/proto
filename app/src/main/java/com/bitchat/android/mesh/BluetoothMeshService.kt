...
class BluetoothMeshService(private val context: Context) {
    ...
    // ESP32 LoRa bridge manager — handles long-range communication via ESP32 + LoRa
    val esp32BridgeManager = ESP32BridgeManager(context, serviceScope)
    ...
    private fun setupDelegates() {
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun sendPacket(packet: BitchatPacket) {
                ...
                // Send via BLE mesh (existing short-range)
                connectionManager.broadcastPacket(RoutedPacket(signedPacket))
                // Also send via ESP32 LoRa bridge (new long-range)
                if (esp32BridgeManager.isConnected()) {
                    esp32BridgeManager.forwardToESP32(signedPacket)
                }
            }
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.bitchat.android.ui.debug.DebugSettingsManager.getInstance()
                        .logPeerConnection(peer ?: "unknown", nick, addr, isInbound = connectionManager.isClientConnection(addr) != true)
                } catch (_: Exception) { }
            }
    ...
    fun startServices() {
        ...
        if (connectionManager.startServices()) {
            ...
            // Start ESP32 LoRa bridge manager — scans for ESP32 nodes in parallel
            esp32BridgeManager.onPacketReceived = { packet, peerID ->
                // Inject LoRa-received packets into the mesh pipeline
                packetProcessor.processPacket(RoutedPacket(packet, peerID, "lora:esp32"))
            }
            esp32BridgeManager.start()
            Log.d(TAG, "ESP32 LoRa Bridge Manager started")
        } else {
    ...
    fun stopServices() {
        ...
        serviceScope.launch {
            ...
            Log.d(TAG, "ESP32 Bridge Manager stopped")
