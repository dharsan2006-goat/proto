...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
        ...
        if (packet.signature != null) {
            ...
            val dataForSigning = packet.toBinaryDataForSigning()
            val sig = packet.signature
            verified = if (dataForSigning != null && sig != null) {
                delegate?.verifyEd25519Signature(sig, dataForSigning, announcement.signingPublicKey) ?: false
            } else false
            if (!verified) {
